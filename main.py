...
# Load the datasets from Kaggle website
...
def information(predicted_dis):
    disease_description = description[description['Disease'] == predicted_dis]['Description'].values[0]
    ...
    disease_workout = workout[workout['disease'] == predicted_dis]['workout'].values.tolist() if not workout[workout['disease'] == predicted_dis].empty else "No workout data available"
    return disease_description, disease_precautions, disease_medications, disease_diet, disease_workout

# Function that passes the user input symptoms to our Model
def predicted_value(patient_symptoms):
    i_vector = np.zeros(len(symptoms_list))
    for i in patient_symptoms:
        if i in symptoms_list:
            i_vector[symptoms_list[i]] = 1
        else:
            return None, "Symptom not found in database"
    return diseases_list[Rf.predict([i_vector])[0]], None

# Function to correct the spellings of the symptom (if any)
def correct_spelling(symptom):
    closest_match, score = process.extractOne(symptom, symptoms_list.keys())
    if score >= 80:
...
def home():
    ...
    if isinstance(symptoms, str):
        ...
        patient_symptoms = [str(s).strip() for s in symptoms]
    else:
        return jsonify({'error': 'Invalid symptoms format'}), 400

    # Correct the spelling of symptoms
    corrected_symptoms = [correct_spelling(symptom) for symptom in patient_symptoms]

    # Predict the disease using corrected symptoms
    predicted_disease, error = predicted_value(corrected_symptoms)
    if error:
        return jsonify({'error': error}), 400

    # Extract information based on predicted disease
